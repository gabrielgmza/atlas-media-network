...
export async function GET(request) {
  try {
    ...
    for (const pub of pubs.rows) {
      try {
        const res = await fetch(`${BASE_URL}/api/newsletter/send`, { method: "POST", headers: { "Content-Type": "application/json", "x-atlas-admin-token": process.env.ATLAS_ADMIN_TOKEN }, body: JSON.stringify({ publicationId: pub.id }) });
