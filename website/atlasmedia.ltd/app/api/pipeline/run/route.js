...
async function getJournalistsForPublication(publicationId) {
  ...
  const result = await db.query(
    `SELECT * FROM public.journalists WHERE publication_id = $1 AND status = 'active'`,
    [publicationId]
  );
  return result.rows;
}
...
async function getPublication(publicationId) {
  const db = getDb();
  const result = await db.query(
    `SELECT p.*, t.name as territory_name FROM public.publications p
...
async function isDuplicateArticle(title, publicationId) {
  ...
  try {
    const result = await db.query(
      `SELECT id FROM public.articles WHERE publication = $1
       AND published_at > NOW() - INTERVAL '24 hours'
       AND lower(title) LIKE lower($2) LIMIT 1`,
      [publicationId, `%${title.slice(0, 30)}%`]
    );
...
Seleccioná las 3 noticias más relevantes para la audiencia de ${publication.name} y asigná el periodista más adecuado a cada una.
  ...
  "selected": [
    {
      ...
      "title": "título",
      "category": "categoría",
      "journalistId": "id exacto",
      "journalistName": "nombre",
      "angle": "ángulo editorial en 1 oración",
...
Respondé SOLO en JSON válido sin markdown:
{
  ...
  "excerpt": "bajada en 1-2 oraciones",
  "content": ["párrafo 1", "párrafo 2", "párrafo 3", "párrafo 4"],
  "seoTitle": "título SEO",
  "seoDescription": "descripción SEO 150 chars"
}`;
...
function makeId(pub) {
  return `${pub.includes("mendoza")?"apm":"ap"}-${Date.now()}-${Math.random().toString(36).slice(2,5)}`;
}
  ...
  try {
    ...
    for (const story of selected) {
      try {
        ...
        const trend = await saveTrendDetection({
          publicationId, pipelineRunId: run.id, title: story.title, summary: story.angle,
          sourceUrl: sourceItem?.sourceUrl, sourceName: sourceItem?.sourceName, category: story.category
        });
    ...
    try {
      const db = getDb();
      await db.query(
        `INSERT INTO public.president_memory (id, type, title, detail, priority, created_at, scope)
         VALUES ($1,'pipeline',$2,$3,'medium',NOW(),'global')`,
...
export async function GET() {
  ...
  const runs = await db.query(
    `SELECT pr.*, p.name as publication_name FROM public.pipeline_runs pr
     LEFT JOIN public.publications p ON pr.publication_id = p.id
