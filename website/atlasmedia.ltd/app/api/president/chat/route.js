  ...
  try {
    switch (toolName) {
      ...
      case "send_executive_report": {
        const res = await fetch(BASE_URL + "/api/cron/executive-report", { headers });
        const data = await res.json();
        return data.ok ? "Reporte ejecutivo enviado. Articulos hoy: " + (data.metrics?.articlesPublished||0) + ". Vistas: " + (data.metrics?.pageviewsToday||0) + ". Email: " + (data.emailSent?"enviado":"no enviado") : "Error: " + data.error;
      }
      default:
        return "Accion desconocida: " + toolName;
    }
  } catch (err) { return "Error ejecutando " + toolName + ": " + err.message; }
}
...
export async function POST(request) {
  try {
    ...
    const tools = [
      { name: "run_pipeline", description: "Ejecuta el pipeline editorial para una publicacion.", input_schema: { type: "object", properties: { publication: { type: "string" } }, required: ["publication"] } },
      ...
      { name: "generate_content", description: "Genera columnas de opinion o articulos de investigacion.", input_schema: { type: "object", properties: { publication: { type: "string" }, contentType: { type: "string", enum: ["opinion","investigation","both"] } }, required: ["publication"] } },
      { name: "send_executive_report", description: "Genera y envia el reporte ejecutivo diario al Founder con metricas del holding.", input_schema: { type: "object", properties: {} } }
    ];
