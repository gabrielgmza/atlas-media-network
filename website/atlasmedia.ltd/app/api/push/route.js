...
function initWebPush() {
  ...
  const email = process.env.FOUNDER_EMAIL || "admin@atlasmedia.ltd";
  if (!publicKey || !privateKey) return false;
  try { webpush.setVapidDetails("mailto:"+email, publicKey, privateKey); return true; } catch { return false; }
}
...
export async function GET(request) {
  try {
    ...
    if (action === "vapid-key") {
      const publicKey = process.env.VAPID_PUBLIC_KEY;
      if (!publicKey) return NextResponse.json({ ok:false, error:"VAPID_NOT_CONFIGURED" });
...
export async function POST(request) {
  try {
    ...
    if (body.action === "send") {
      ...
      if (!configured) return NextResponse.json({ ok:false, error:"VAPID_NOT_CONFIGURED" }, { status:503 });
      const { publicationId, title, body:msgBody, url, breaking } = body;
      const query = publicationId ? `SELECT * FROM public.push_subscriptions WHERE publication_id=$1 AND status='active'` : `SELECT * FROM public.push_subscriptions WHERE status='active'`;
      const subs = publicationId ? await db.query(query,[publicationId]) : await db.query(query);
      const payload = JSON.stringify({ title, body:msgBody, url, breaking:breaking||false, tag:"atlas-"+Date.now() });
      let sent=0, failed=0;
      for (const sub of subs.rows) {
        try {
          ...
          if (err.statusCode===410) await db.query(`UPDATE public.push_subscriptions SET status='inactive' WHERE endpoint=$1`,[sub.endpoint]);
          failed++;
        }
      }
      return NextResponse.json({ ok:true, sent, failed, total:subs.rows.length });
    }
